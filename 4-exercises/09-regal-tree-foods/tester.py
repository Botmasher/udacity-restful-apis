...
try:
	h = Http()
	h.add_credentials("%sasdf123" % token,'blank')
	url = address + '/products'
	data = dict(name = "apple", category = "fruit", price= "$.99")
	resp, content = h.request(url,'POST', body = json.dumps(data), headers = {"Content-Type" : "application/json"})
	if resp['status'] == '200':
		raise Exception('Received a successful status code of %s with a bad token' % resp['status'])
except Exception as err:
	print("Test 4 FAILED: Could access with invalid token!")
	print(err.args)
	sys.exit()
else:
	print("Test 4 PASS: Succesfully blocked access with invalid token")
